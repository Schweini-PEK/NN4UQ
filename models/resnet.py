...
class BasicResBlock(BasicModule):
    def __init__(self, in_dim, h_dim, out_dim, n_hidden_layer, activation='Swish'):
        ...
        # self.layer1 = nn.Sequential(nn.Linear(in_dim, h_dim), self.activation())
        # self.layer2 = nn.Sequential(nn.Linear(h_dim, h_dim), self.activation())
        # self.layer3 = nn.Sequential(nn.Linear(h_dim, h_dim), self.activation())
        self.layer_dp = nn.Dropout(p=0.5)
        # self.layer5 = nn.Sequential(nn.Linear(h_dim, out_dim))
...
class ResNet(BasicModule):
    def __init__(self, block=BasicResBlock, in_dim=3, n_hidden=30, out_dim=1, n_h_layers=3):
...
class RSResNet(BasicModule):
    def __init__(self, block=BasicResBlock, in_dim=3, h_dim=20, out_dim=1, k=3, n_h_layers=3):
